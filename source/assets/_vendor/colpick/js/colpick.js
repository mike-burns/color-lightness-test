...
(function ($) {
  var colpick = function () {
    var
      ...
      blur = function (ev) {
        $(this).parent().removeClass('colpick-focus');
      },
      focus = function () {
        $(this).parent().parent().data('colpick').fields.parent().removeClass('colpick-focus');
        $(this).parent().addClass('colpick-focus');
      },
      ...
      downIncrement = function (ev) {
        ...
        var current = {
          el: $(this).parent().addClass('colpick-slider'),
      ...
      upIncrement = function (ev) {
        change.apply(ev.data.field.get(0), [true]);
        ev.data.el.removeClass('colpick-slider').find('input').focus();
  ...
  init: function (opt) {
    ...
    return this.each(function () {
      ...
      if (!$(this).data('colpickId')) {
        ...
        var id = 'collorpicker-' + parseInt(Math.random() * 1000);
        $(this).data('colpickId', id);
        //Set the tpl's ID and get the HTML
        var cal = $(tpl).attr('id', id);
        //Add class according to layout
        cal.addClass('colpick-'+options.layout+(options.submit?'':' colpick-'+options.layout+'-ns'));
        //Add class if the color scheme is not default
        if(options.colorScheme != 'light') {
          cal.addClass('colpick-'+options.colorScheme);
        }
        //Setup submit button
        cal.find('div.colpick-submit').html(options.submitText).click(clickSubmit);
        //Setup input fields
        options.fields = cal.find('input').change(change).blur(blur).focus(focus);
        cal.find('div.colpick-field-arrs').mousedown(downIncrement).end().find('div.colpick-current-color').click(restoreOriginal);
        //Setup hue selector
        options.selector = cal.find('div.colpick-color').mousedown(downSelector);
        options.selectorIndic = options.selector.find('div.colpick-selector-outer');
        //Store parts of the plugin
        options.el = this;
        options.hue = cal.find('div.colpick-hue-arrs');
        ...
        if(ngIE) {
          ...
          huebar.css({'background':'linear-gradient(to bottom,'+stopList+')'});
              huebar.css({'background':'-moz-linear-gradient(top,'+stopList+')'});
                ...
                cal.find('div.colpick-hue').mousedown(downHue);
                options.newColor = cal.find('div.colpick-new-color');
                options.currentColor = cal.find('div.colpick-current-color');
